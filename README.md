...
```sql
SELECT 
top 1[product category], sum(sales) as [Total Sales]
from [KMS data] 
...
```sql
select
top 10 [Customer Name],SUM(Sales) as totalsales
from[dbo].[KMS data]
...
```sql
select
top 1[Ship Mode], sum([shipping cost])as [Total shipping cost] from [KMS data]
group by[Ship Mode]
...
```sql
select
 top 10 [customer name],[Product Sub-Category],sum (sales) as [Total Sales]
from [kms data]
...
```sql
select
top 1 [customer name],count([Order ID])as [Total order]
from [KMS data]
...
```sql
select
top 1 [customer name],[customer segment], sum(profit)as [Total profit]
from [KMS data]
...
```sql
select
 [Order ID], [customer name],[customer segment],[dbo].[Order_Status].[Status]
from [dbo].[KMS data]
...
```sql
select
[order priority],[ship mode], round (sum(sales-profit),2) as [ Estimated shipping cost], 
